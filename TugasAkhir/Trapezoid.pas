...
begin
  ...
  outtextxy(xmin + OFFSET_X, ymin - 20, 'Trapesium Asli');
end;
...
begin
  ...
  { Definisikan trapesium }
  numPoints := 4;
  inPoly[1].x := 50;   inPoly[1].y := 50;   { Titik kiri atas }
  inPoly[2].x := 250;  inPoly[2].y := 50;   { Titik kanan atas }
  inPoly[3].x := 300;  inPoly[3].y := 250;  { Titik kanan bawah }
  inPoly[4].x := 20;   inPoly[4].y := 250;  { Titik kiri bawah }
  
  { Gambar window clipping }
  DrawClippingWindow;
  
  { Gambar trapesium asli di sebelah kanan }
  setcolor(LIGHTGRAY);
